  ...
  <script type="text/javascript">
    ...
    function makeView(data){  // data=[{  },{    },{    },,,]
    	...
    	$.each(data, function(index,obj){  // obj={"idx":5, "title":"게시판"~~}
        	...
        	listHtml+="<br/>"; 	
       		listHtml+="<span id='ub"+obj.idx+"'><button class='btn btn-success btn-sm' onclick='goUpdateForm("+obj.idx+")'>수정화면</button></span>&nbsp;"; //&nbsp; : 공백 한칸
     		listHtml+="<button class='btn btn-warning btn-sm' onclick='goDelete("+obj.idx+")'>삭제</button>";
           }else{
           	listHtml+="<br/>"; 	
       		listHtml+="<span id='ub"+obj.idx+"'><button disabled class='btn btn-success btn-sm' onclick='goUpdateForm("+obj.idx+")'>수정화면</button></span>&nbsp;"; //&nbsp; : 공백 한칸
    ...
    function goInsert(){
    	...
    	var fData=$("#frm").serialize(); // 폼 안에 있는 파라미터 전부 가져옴 (직렬로)
    	// alert(fData);
    	$.ajax({
    		url : "board/new",
    		type : "post",
    	...
    	//$("#content").val("");
    	//$("#writer").val("");
    	$("#fclear").trigger("click"); // trigger:추적해서 실행
    }
...
<div class="container">
  ...
  <div class="panel panel-default">
    ...
    <div class="panel-footer">인프런_스프1탄_하방</div>
  </div>
</div>
